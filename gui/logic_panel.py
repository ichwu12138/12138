...
from utils.expression_formatter import ExpressionFormatter
...
class LogicPanel(ttk.Frame):
    ...
    def __init__(self, parent, logic_builder: LogicBuilder, config_processor=None, bom_processor=None, main_window_ref=None):
        ...
        # 创建表达式格式化器实例
        self.formatter = ExpressionFormatter(config_processor, bom_processor)
    ...
    def _create_widgets(self):
        ...
        center_btn_frame.pack(side=LEFT, padx=20)
        
        # 横转竖按钮
        self.horizontal_to_vertical_btn = ttk.Button(
            center_btn_frame,
            text="横→竖",
            command=self._convert_horizontal_to_vertical,
            style="Main.TButton",
            width=12
        )
        self.horizontal_to_vertical_btn.pack(side=LEFT, padx=2)
        
        # 竖转横按钮
        self.vertical_to_horizontal_btn = ttk.Button(
            center_btn_frame,
            text="竖→横",
            command=self._convert_vertical_to_horizontal,
            style="Main.TButton",
            width=12
        )
        self.vertical_to_horizontal_btn.pack(side=LEFT, padx=2)
        ...
        self.toggle_comments_btn.pack(side=LEFT, padx=2)
    ...
    def _convert_horizontal_to_vertical(self):
        """将横式表达式转换为竖式表达式"""
        try:
            # 获取当前表达式
            current_expr = self.expr_text.get("1.0", "end-1c").strip()
            if not current_expr:
                from utils.message_utils_tk import show_warning
                show_warning("empty_expression")
                return
                
            # 检查是否包含蕴含符号
            if "→" not in current_expr:
                from utils.message_utils_tk import show_warning
                show_warning("error_missing_implication")
                return
                
            # 弹出格式选择对话框
            format_choice = self._show_format_choice_dialog()
            if not format_choice:
                return
                
            # 使用格式化器转换
            vertical_expr = self.formatter.horizontal_to_vertical(
                current_expr, 
                format_style=format_choice,
                include_comments=self.show_comments
            )
            
            # 更新文本框
            self.expr_text.delete("1.0", "end")
            self.expr_text.insert("1.0", vertical_expr)
            
            self.logger.info(f"成功将横式表达式转换为竖式 ({format_choice})")
            
        except Exception as e:
            self.logger.error(f"横转竖转换时出错: {str(e)}", exc_info=True)
            from utils.message_utils_tk import show_error
            show_error("conversion_error", error=str(e))
    ...
    def _convert_vertical_to_horizontal(self):
        ...
        try:
            ...
            if "\n" not in current_expr:
                from utils.message_utils_tk import show_warning
                show_warning("not_vertical_format")
                return
                
            # 使用格式化器转换
            horizontal_expr = self.formatter.vertical_to_horizontal(current_expr)
            
            # 更新文本框
            self.expr_text.delete("1.0", "end")
            self.expr_text.insert("1.0", horizontal_expr)
            
            self.logger.info("成功将竖式表达式转换为横式")
            
        except Exception as e:
            self.logger.error(f"竖转横转换时出错: {str(e)}", exc_info=True)
            from utils.message_utils_tk import show_error
            show_error("conversion_error", error=str(e))
    ...
    def _toggle_comments(self):
        ...
        try:
            ...
            if self.show_comments:
                self.toggle_comments_btn.configure(text="隐藏注释")
            else:
                self.toggle_comments_btn.configure(text="显示注释")
            ...
            # 如果是竖式格式，重新应用注释状态
            if "\n" in current_expr:
                # 先转换为横式，再转换回竖式以应用新的注释状态
                horizontal_expr = self.formatter.vertical_to_horizontal(current_expr)
                vertical_expr = self.formatter.horizontal_to_vertical(
                    horizontal_expr,
                    format_style="grouped",  # 默认使用分组格式
                    include_comments=self.show_comments
                )
                self.expr_text.delete("1.0", "end")
                self.expr_text.insert("1.0", vertical_expr)
                ...
                # 如果是横式格式，直接添加或移除内联注释
                if self.show_comments:
                    commented_expr = self.formatter.format_with_descriptions(current_expr, "inline")
                else:
                    # 移除注释
                    commented_expr = re.sub(r'\s*#[^#]*', '', current_expr)
                    
                self.expr_text.delete("1.0", "end")
                self.expr_text.insert("1.0", commented_expr)
                
            self.logger.info(f"切换注释显示状态: {self.show_comments}")
            
        except Exception as e:
            self.logger.error(f"切换注释时出错: {str(e)}", exc_info=True)
            
    def _show_format_choice_dialog(self) -> Optional[str]:
        """显示格式选择对话框
        
        Returns:
            Optional[str]: 选择的格式 ("grouped" 或 "expanded")，None表示取消
        """
        import tkinter as tk
